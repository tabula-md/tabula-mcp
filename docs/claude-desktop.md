...
Open `dist/tabula-mcp-0.2.2.mcpb`, install it, and restart Claude Desktop. Packaging also creates stable `dist/tabula-mcp.mcpb` and `dist/tabula-mcp.mcpb.sha256` aliases for the latest-release download. When replacing an older build, a full restart is required because Claude Desktop caches MCP App resources and tool definitions.
